...
const Product = () => {
  ...
  const isMatched = (isId) => {
    return theProductContext.selected.id === isId
      ? "chevron-active"
      : "chevron-inactive";
  };
  ...
  const rates = (num) => {
    ...
    if (tmpRate > 1 && num > 1 && tmpRate <= 2 && num <= 2) {
      return true;
    }
    if (tmpRate > 2 && num > 2 && tmpRate <= 3 && num <= 3) {
      return true;
    }
    if (tmpRate > 3 && num > 3 && tmpRate <= 4 && num <= 4) {
      return true;
    }
    if (tmpRate > 4 && num > 4 && tmpRate <= 5 && num <= 5) {
      return true;
    }
  };
  ...
  return (
    <div>
      ...
      <div className="product-frame">
        <div className="w-6-12 product-container">{product()}</div>
        {theProductContext?.selected?.id && (
          <div className="w-6-12 product-details">
            <div>
              ...
              <div className="ratings">
                ...
                <div className="count">
                  ({theProductContext.selected?.rating?.count})
                </div>
              </div>
              <div>
